...
FingerPrintGenerator = (function(){
  ...
  function getMaterialOfFingerprintWithOptions({ activeOptions = [] } = {}) {
    const excludeOptions = activeOptions.length === 0 ? []:allOptions.filter((option) => activeOptions.indexOf(option) === -1);
  ...
  const getComponents = getMaterialOfFingerprintWithOptions;
  return {
    getFingerprint,
    getComponents
  };
